...
public class StringTools {
	...
	public static String getPayRamCode() {
		...
		return StringTool.padRight(String.valueOf(random.nextInt(1000)), 3, '0');
	}
	 ...
	 * @return
	 */
	public static String getFommatMac(String mac) {
		try {
			...
			return sbBuffer.substring(0, sbBuffer.length() - 1).toUpperCase();
		} catch (Exception e) {
			e.printStackTrace();
			return mac;
		}
	 ...
	 * @return
	 */
	public static String getFommatMac(String mac, String spit) {
		try {
			if (StringTool.isNullOrEmpty(mac))
				return null;
			String processMac = mac.replace("-", "");
	...
	public static Integer getRateFrom2Integer(Integer value1, Integer value2) {
		if (null == value1 || null == value2 || 0 >= value2 || 0 > value1) {
			return 0;
		}
		double d = (float) value1 / value2;
		d = d * 100;
	...
	public static void addSqlWhereToMyBatisWhere(MyBatisWhere myBatisWhere, SqlWhere sqlWhere) {
		if (null != myBatisWhere && null != myBatisWhere.getWheres() && myBatisWhere.getWheres().size() > 0) {
			SqlWhere filterSqlWhere = myBatisWhere.getWheres().get(0);
			if (null != sqlWhere) {
				for (int i = 0; i < sqlWhere.getConditions().size(); i++) {
					filterSqlWhere.addCondition(sqlWhere.getConditions().get(i));
				}
			}
		} else {
			if (sqlWhere != null) {
	...
	public static String getMonthFirstDate(Date date) {
		...
		String result = DateUtil.getFormatDate(cal_1.getTime(), "yyyy-MM-dd");
		return result;
	}
	...
	/**
	 * Description：计算2个时间的天数，得到结果列表:2016-03-01,2016-03-02,..,如果是当月就不要今天之后的，包括今天
	 * 
	 * @param month_yyyy_mm
	 *            2016-07
	 * @return
	 *
	 */
	public static List<String> calculateMonthDaysWithoutTomorrow(String month_yyyy_mm) {
		List<String> result = new ArrayList<String>();
		try {
			Date monthdate = DateUtil.parseDate(month_yyyy_mm, "yyyy-MM");
			String startdate = getMonthFirstDate(monthdate);
			String enddate = getMonthEndDate(monthdate);
			String now_yyyy_mm_dd = DateUtil.getCurrentDate("yyyy-MM-dd");

			Calendar begin = new GregorianCalendar();
			Calendar end = new GregorianCalendar();
			begin.setTime(DateUtil.parseDate(startdate, "yyyy-MM-dd"));
			end.setTime(DateUtil.parseDate(enddate, "yyyy-MM-dd"));

			while (0 <= enddate.compareTo(startdate) && 0 < now_yyyy_mm_dd.compareTo(startdate)) {
				result.add(startdate);
				begin.add(Calendar.DATE, 1);
				startdate = DateUtil.getFormatDate(begin.getTime(), "yyyy-MM-dd");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * System.out.println(checkPassword("123456",6,16));
		 */
		// System.out.println(changePhoneString("1234567"));
