...
public class ExcelUtil {
	...
	public void export2Xlxs(List lstData, Map<String, String> mapHeader, String xlsPath)
			throws IOException, NoSuchFieldException, SecurityException, IllegalArgumentException,
			IllegalAccessException, EncryptedDocumentException, InvalidFormatException {
		...
		if (!file.exists()) {
			...
			Iterator<Map.Entry<String, String>> entries = mapHeader.entrySet().iterator();
			...
			wb = new XSSFWorkbook(new FileInputStream(xlsPath));
		...
		for (int i = 0; i < lstData.size(); i++) {
			...
			while (entries.hasNext()) {
				...
				if (type.equals("class java.lang.String")) { // 如果type是类类型，则前面包含"class
																// "，后面跟类名
					String username = (String) field.get(model);
					cell.setCellValue(username);

				} else if (type.equals("class java.lang.Integer")) {

					cell.setCellValue((Integer) field.get(model));
				} else if (type.equals("class java.lang.Boolean")) {
					cell.setCellValue((Boolean) field.get(model));

				} else if (type.equals("class java.util.Date")) {
					Date date = (Date) field.get(model);
					if (date != null) {
						cell.setCellValue(DateUtil.getFormatDate(date, "yyyy-MM-dd HH:mm:ss"));
					}

				}

				else if (type.equals("class java.math.BigDecimal")) {
					BigDecimal balance = (BigDecimal) field.get(model);
					if (balance != null)
						cell.setCellValue(balance.toPlainString());

				} else if (type.equals("int")) {
					cell.setCellValue(field.getInt(model));

				} else if (type.equals("float")) {

					cell.setCellValue((double) (field.getFloat(model)));
				} else if (type.equals("double")) {

					cell.setCellValue(field.getDouble(model));
				}
				colIndex++;
			}
			rowIndex++;
	...
	public int export2XlsWithInfo(List lstData, Map<String, ExportInfo> mapHeader, String xlsPath)
			throws IOException, NoSuchFieldException, SecurityException, IllegalArgumentException,
			IllegalAccessException, EncryptedDocumentException, InvalidFormatException {

		// 有些导出太耗性能，保证同时导出功能最多只有3个导出操作，当超过3个给客户端提示稍后再导
		...
		if (!file.exists()) {
			...
			Iterator<Map.Entry<String, ExportInfo>> entries = mapHeader.entrySet().iterator();
			Row row = sheet.createRow(0);
			// 增加表头
			while (entries.hasNext()) {

				Map.Entry<String, ExportInfo> entry = entries.next();
		...
		for (int i = 0; i < lstData.size(); i++) {
			...
			while (entries.hasNext()) {
				...
				try {
					...
					Object val = ReflectionUtils.invokeMethod(method, model);
					// field.setAccessible(true);
					// String type =field.getGenericType().toString();
	...
	public int export2XlsWithInfo_Map(List<DataRow> rows, Map<String, ExportInfo> mapHeader, String xlsPath)
			throws IOException, NoSuchFieldException, SecurityException, IllegalArgumentException,
			IllegalAccessException, EncryptedDocumentException, InvalidFormatException {
		// 有些导出太耗性能，保证同时导出功能最多只有3个导出操作，当超过3个给客户端提示稍后再导
		// 扩展名为空时，
		...
		for (int i = 0; i < rows.size(); i++) {
			Map<String, Object> onemap = rows.get(i);
			colIndex = 0;
			Iterator<Map.Entry<String, ExportInfo>> entries = mapHeader.entrySet().iterator();
			Row row = sheet.createRow(rowIndex);
			while (entries.hasNext()) {
				...
				Cell cell = row.createCell(colIndex);
				String colname = entry.getKey();
				String fieldvalue = onemap.get(colname) + "";
	...
	public static void main(String[] args) {
		...
		List appuser = new ArrayList<AppUser>();
		for (int i = 0; i < 100; i++) {
			AppUser user = new AppUser();
			user.setUsername(i + "");
			user.setMemshipdate(new Date());
			user.setBalance(new BigDecimal(i * i + 0.5));
			appuser.add(user);
		}
		Map<String, String> map = new HashMap<String, String>();
		map.put("username", "用户名");
		map.put("memshipdate", "wifi到期时间");
		map.put("balance", "余额");
		try {
			excelUtil.export2Xlxs(appuser, map, "d:/test.xlsx");
